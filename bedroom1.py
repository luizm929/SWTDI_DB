...
# JulTime.CalJT() is not in the GitHub repository because I do not own it.
a = JulTime.CalJT()
...
# This program is attached to code from Jose Tabarez                                                                             ##
...
##      Taken from: Programming in Python 3: A complete Introduction to the Python Language(2nd edition)
##This is the reason I did not use the following method to insert multiple rows
##into database. 
##CODE
## args_str = ','.join(cur.mogrify("(%s,%s,%s,%s,%s,%s,%s,%s,%s)", x) for x in tup)
## cur.execute("INSERT INTO table VALUES " + args_str) 
##CODE ENDS
## where tup is a tuple and args_str in my data.
##---------------------------------------------------------------------------------------------------------##
...
def main():
    ...
    # Randomise Occupancy we create a few just to not get the same value for all outlets
    ...
    conn_string = "host='128.123.130.86' dbname='masterdb' user='edo' password=This is where password goes"
    ...
    #TODO Need to avoid repeating code by writing a function which inserts values into all smart outlets.
    ## SMART OUTLET 1   ##
    ...
    cursor.execute("INSERT INTO outlet1_2017_04 (Device_id, Voltage, Current, Phase, Temperature, Ocupancy, Power, "
                   "Outlet1Timestamp, RPITimestamp, MainServerTimestamp)  VALUES (%s,%s, %s, %s, %s, %s, %s, %s, %s, %s)",
                   ("Ox01", voltage, current, phase, temp, oc1, power,a,a,a))
    ...
    ## We need to commit all changes else nothing gets saved on table
    conn.commit()
    cursor.close()
    conn.close()
